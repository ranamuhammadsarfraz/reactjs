...
function Number21() {
  ...
  async function compareMethod(event: any) {
    ...
    try {
      ...
      if (!dataIt.data.similarity) {
        ...
        setResult(
          (dataIt.data.similarity.toFixed(3) * 100).toFixed(0) + "% similar",
        );
        setLoad(false);
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    setText1length(
      text1.split(" ").length.toString() + " words in First Section",
    );
  }, [text1]);
  useEffect(() => {
    if (text2.length == 0) {
    ...
    setText2length(
      text2.split(" ").length.toString() + " words in First Section",
    );
  }, [text2]);
  return (
    <main id={styles.main}>
      <h1>Text Similarity WebTool</h1>
      <p style={{ backgroundColor: "silver", width: "max-content" }}>
        Maximum words 5000
      </p>
      <p>{text1Length}</p>
      <p>{text2Length}</p>
      <form onSubmit={compareMethod}>
        <textarea
          ...
          disabled={load}
          value={text1}
          onChange={(turn) => setText1(turn.target.value)}
          required
        ...
        <textarea
          ...
          disabled={load}
          value={text2}
          onChange={(turn) => setText2(turn.target.value)}
          required
      ...
      <span>
        <div>
          API (Application programming Interface) is provided by third party.
        </div>
