    ...
    <main id={styles.main}>
      <div>
        ...
        <form onSubmit={myMethod}>
          <input
            disabled={loading}
            value={input}
            onChange={(turn) => setInput(turn.target.value)}
            type="text"
            placeholder="Country *"
            required
          />
      ...
      <section>
        <p>
          {!loading ? alertDiv : (
            <span>
              <GiSandsOfTime className={styles.loading} />
            </span>
          )}
        </p>
        {(myData == "") ? "" : myData.map(
          (data: { "0": string; "1": string | number }, index: number) => {
            return (
              <code key={index}>
                <button>
                  <main>{data["0"]}</main>
      ...
      <span>
        ...
        <p>
          Data is controlled by third party API (Application programming
          interface).
        </p>
      </span>
    </main>
